 ...
 * - ownership is transferred to multisig treasury
 * - multisig treasury nominates the subcommittee address
 * - multisig treasury funds the contract and mints tSWRV tokens
...
contract tSWRV is ERC20("Swerve Treasury Promissory Token", "tSWRV") {
    ...
    event PromissoryTokensMinted(uint date, uint256 _amount);
    event GrantCreated(uint date, uint256 _grantID, address _recipient, uint256 _amount, uint256 _bonusPercentage, uint256 _redemptionDelay);
    event PromissoryTokensRedeemed(uint date, uint256 grantID, address _beneficiary, uint256 _amount, uint256 payout);
    event OwnershipTransferred(uint date, address _previousOwner, address _newOwner);
    event SubcommitteeAddressChanged(uint date, address _previousSubcommittee, address _newSubcommittee);
    event FunctionUnlockStarted(uint date, TimelockedFunctions _fn);
    event FunctionLocked(uint date, TimelockedFunctions _fn);
    ...
    function setSubcommitteeAddress(address _subcommitteAddress) public onlyOwner notLocked(TimelockedFunctions.SETSUBCOM) {
        emit SubcommitteeAddressChanged(now, subcommitteeAddress, _subcommitteAddress);
    ...
    function transferOwnership(address _newOwner) public onlyOwner notLocked(TimelockedFunctions.CHGOWNERSHIP) {
    ...
    function createGrant(address _recipient, uint256 _amount, uint256 _bonusPercentage, uint256 _redemptionDelay) public {
        require(_msgSender() == subcommitteeAddress, "Only the subcommittee can create a grant");
        ...
        emit GrantCreated(block.timestamp, counter, _recipient, _amount, _bonusPercentage, _redemptionDelay);
    }
    ...
    function redeem(uint256 _grantID, uint256 _amount) public {
        ...
        require(grants[_grantID].recipient == _msgSender(), "Only the grant recipient can redeem its tSWRV tokens");
        require(balanceOf(_msgSender()) >= _amount, "Insufficient tSWRV balance");
        require(grants[_grantID].redemptionDate <= block.timestamp, "Grant is not yet redeemable for swUSD");
        ...
        require(getTreasuryBalance() >= payout, "Insufficient swUSD  balance in treasury");
        ...
        emit PromissoryTokensRedeemed(now, _grantID, _msgSender(), _amount, payout);
    }
